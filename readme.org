...
You'll find it here: https://codeberg.org/rubenphilipp/hermes
...
=ffmpeg= is required to convert the video file to HLS files, which are used by
the media player for variable resolution playback.

On a Mac (with homebrew), use:
